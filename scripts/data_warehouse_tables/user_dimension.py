...
def get_users():
    users_path = repo_root + "/data/dummy_data/example_SQS_batch_event_users.json"
    users_list = []
    with open(users_path, 'r') as f:
        message_batch = json.load(f)
        for message in message_batch["Records"]:
            user_list = ast.literal_eval(message["body"])
...
def main():
    user_list = get_users() # get users to get info on from SQS message event
    current_users, current_user_dim_df = get_current_user_dim() # get current users we have data for
    new_users = get_only_new_users(user_list, current_users)
