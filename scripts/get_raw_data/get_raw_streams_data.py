...
def main():
    ...
    unique_file_identifier = str(random.randint(1, 1000000))  # proxy for unique identifier of data we will get from lambda function
    time_identifier = f"{day_date_id}_{time_of_day_id}"
    folder_path = repo_root + f"/data/twitch_project_raw_layer/raw_streams_data/{time_identifier}/"
    file_name = f"raw_streams_data_X{unique_file_identifier}X_{time_identifier}.json"
    raw_streams_data_path = os.path.join(folder_path, file_name)
    os.makedirs(folder_path, exist_ok=True)  # Create the directory (and any necessary parent directories)

    with open(raw_streams_data_path, 'w') as json_file:  # Write the data as a JSON file
